...
public class PageHelper {
	...
	public StringBuffer getPageUrl(SearchVO searchVO, int totalRow) {
		...
		if(currentBlock > 0){
			pageHtml.append("<div class='pagingBorder'><div class='backPage'><a href=\"generalForum?page="+(currentPage-1)+ smsg + "\" style='text-decoration: none;font-weight:bold;color:#7e7e7e;' ><</a></div>");
		}else{
		...
		for(int i = startPage ; i <= endPage ; i++) {
			if(i == currentPage){
				...
				if(i == startPage) {
					pageHtml.append("<div class='paging' style='border-left:none;'><a href=\"generalForum?page=" + i + smsg + "\" style='text-decoration: none;color:black;'>" +i+"</a></div>");
				}else{
					pageHtml.append("<div class='paging'><a href=\"generalForum?page=" + i + smsg + "\" style='text-decoration: none; color:black;'>" +i+"</a></div>");
				}
		...
		if(currentBlock == (endPage-1)){
			...
			pageHtml.append("<div class='frontPage'><a href=\"generalForum?page="+(currentPage+1)+ smsg + "\" style='text-decoration: none;font-weight:bold;color:#7e7e7e;'>></a></div></div>");
		}
		return pageHtml;
	}
